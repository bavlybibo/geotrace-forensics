"""Report export engine implementation.

Moved from app.core.report_service.service during v12.10.2 organization-only refactor.
The public import path remains app.core.report_service.service and app.core.report_service.
"""
...
class ReportService:
    ...
    def _case_metrics(self, records: List[EvidenceRecord]) -> dict:
        ...
        hidden_count = sum(1 for record in records if record.hidden_code_indicators or record.hidden_suspicious_embeds or getattr(record, "pixel_hidden_score", 0) >= 40 or getattr(record, "pixel_lsb_strings", []))
            ...
            payload.append(
                {
                    ...
                    "image_detail_profile": {
                        "label": record.image_detail_label,
                        "confidence": record.image_detail_confidence,
                        "summary": self._redact_freeform_text(record.image_detail_summary, privacy_level),
                        "cues": self._redact_collection(record.image_detail_cues, privacy_level),
                        "layout_hints": self._redact_collection(record.image_layout_hints, privacy_level),
                        "object_hints": self._redact_collection(record.image_object_hints, privacy_level),
                        "quality_flags": self._redact_collection(record.image_quality_flags, privacy_level),
                        "metrics": record.image_detail_metrics,
                        "attention_regions": record.image_attention_regions,
                        "scene_descriptors": self._redact_collection(record.image_scene_descriptors, privacy_level),
                        "analysis_methodology": self._redact_collection(record.image_analysis_methodology, privacy_level),
                        "performance_notes": self._redact_collection(record.image_performance_notes, privacy_level),
                        "limitations": self._redact_collection(record.image_detail_limitations, privacy_level),
                        "next_actions": self._redact_collection(record.image_detail_next_actions, privacy_level),
                    },
                    "map_intelligence": {
                        ...
                        "answer_readiness": {
                            "score": getattr(record, "map_answer_readiness_score", 0),
                            "label": getattr(record, "map_answer_readiness_label", "Not answer-ready"),
                            "anchor_status": self._redact_freeform_text(getattr(record, "map_anchor_status", "No stable map/location anchor recovered."), privacy_level, replacement="[REDACTED_LOCATION_CONTEXT]"),
                        },
                        "visual_profile": getattr(record, "map_visual_profile", {}),
                        ...
                        "extraction_plan": self._redact_collection(getattr(record, "map_extraction_plan", []), privacy_level, replacement="[REDACTED_LOCATION_ACTION]"),
                    ...
                    "hidden": {
                        ...
                        "pixel_forensics": {
                            ...
                            "summary": self._redact_freeform_text(record.pixel_hidden_summary, privacy_level),
                            "indicators": self._redact_collection(record.pixel_hidden_indicators, privacy_level),
                            "lsb_strings": self._redact_collection(record.pixel_lsb_strings, privacy_level, replacement="[REDACTED_PIXEL_STRING]"),
                            "alpha_findings": self._redact_collection(record.pixel_alpha_findings, privacy_level),
                            "channel_notes": record.pixel_channel_notes,
                            "metrics": record.pixel_hidden_metrics,
                            "limitations": record.pixel_hidden_limitations,
                            "next_actions": self._redact_collection(record.pixel_hidden_next_actions, privacy_level),
                        },
                    },
                    "visible_text": {
    ...
    def export_ai_guardian_summary(self, records: List[EvidenceRecord], case_id: str, case_name: str, *, privacy_mode: bool = True, privacy_level: str | None = None) -> Path:
        ...
        if map_records:
            for record in sorted(map_records, key=lambda item: (-item.map_intelligence_confidence, item.evidence_id))[:20]:
                ...
                lines.append(f"- {record.evidence_id}: {record.map_app_detected} | {record.map_type} | route={route} ({record.route_confidence}%) | city={city} | area={area} | confidence={record.map_intelligence_confidence}% | answer={getattr(record, 'map_answer_readiness_label', 'Not answer-ready')} ({getattr(record, 'map_answer_readiness_score', 0)}%)")
    ...
    def export_ctf_geolocator_writeup(self, records: List[EvidenceRecord], case_id: str, case_name: str, *, privacy_level: str | None = None) -> Path:
        """Export the CTF GeoLocator writeup as a first-class package artifact."""
        privacy_level = privacy_level or "redacted_text"
        output = self.export_dir / f"ctf_geolocator_writeup_{self._privacy_suffix(privacy_level)}.md"
        lines = [
            "# GeoTrace OSINT / CTF GeoLocator Writeup",
            "",
            f"Case ID: `{html.escape(case_id)}`",
            f"Case Name: {html.escape(case_name)}",
            f"Privacy level: `{privacy_level}`",
            "",
            "> Offline-first CTF/geolocation analysis. Treat OCR/map/visual candidates as leads until corroborated by GPS, map URL, browser/app history, or analyst verification.",
            "",
        ]
        ordered = sorted(records, key=lambda item: (-getattr(item, "location_solvability_score", 0), getattr(item, "evidence_id", "")))
        for record in ordered:
            safe_name = self._safe_file_name(record, privacy_level)
            lines.extend([
                f"## {record.evidence_id} — {safe_name}",
                "",
                f"- Location solvability: **{self._redact_text(str(record.location_solvability_label), privacy_level)}** ({record.location_solvability_score}%)",
                f"- Best estimate: **{self._safe_geo_display(str(record.location_estimate_label), privacy_level)}** ({record.location_estimate_confidence}%, {record.location_estimate_scope})",
                f"- Map intelligence: {record.map_app_detected} / {record.map_type} / route={'yes' if record.route_overlay_detected else 'no'} / confidence={record.map_intelligence_confidence}%",
                f"- OCR: {record.ocr_confidence}% — {self._redact_text(str(record.ocr_note), privacy_level)}",
                "",
                "### Candidate ranking",
            ])
            candidates = [c for c in (record.geo_candidates or []) if isinstance(c, dict)]
            if candidates:
                for candidate in candidates[:8]:
                    name = self._redact_text(str(candidate.get("name", "candidate")), privacy_level)
                    basis = self._join_redacted([str(x) for x in candidate.get("basis", [])], privacy_level, limit=4, replacement="[REDACTED_BASIS]")
                    lines.append(
                        f"- **{name}** — {candidate.get('level', 'level')} / {candidate.get('confidence', 0)}% / {candidate.get('evidence_strength', 'weak_signal')} / status={candidate.get('status', 'needs_review')} / basis={basis}"
                    )
            else:
                lines.append("- No candidate ranking was generated.")
            clues = [c for c in (record.ctf_clues or []) if isinstance(c, dict)]
            lines.extend(["", "### Key clues"])
            if clues:
                for clue in clues[:10]:
                    value = self._redact_text(str(clue.get("value", "")), privacy_level)
                    lines.append(f"- {clue.get('clue_type', 'clue')}: {value} ({clue.get('source', 'unknown')}, {clue.get('confidence', 0)}%)")
            else:
                lines.append("- No CTF clue cards available.")
            if record.map_evidence_ladder:
                lines.extend(["", "### Map evidence ladder"])
                for item in record.map_evidence_ladder[:10]:
                    lines.append(f"- {self._redact_text(str(item), privacy_level)}")
            if record.ctf_search_queries and not self._is_strict_redacted(privacy_level):
                lines.extend(["", "### Manual search pivots"])
                lines.extend(f"- {query}" for query in record.ctf_search_queries[:10])
            elif record.ctf_search_queries:
                lines.extend(["", "### Manual search pivots", "- [REDACTED_SEARCH_PIVOTS] available in Internal Full export only."])
            lines.extend(["", "### Analyst-safe next actions"])
    ...
    def export_package_manifest(self, payload: dict, *, privacy_level: str | None = None) -> Path:
        ...
        try:
            ...
            (self.export_dir / "export_manifest.sha256").write_text(f"{digest}  export_manifest.json\n", encoding="utf-8")
        except Exception as exc:
            self.logger.warning("Could not write export manifest signature sidecar: %s", exc)
        return output
        ...
        for record in records:
            ...
            appendix_blocks.append(
                ...
                <div class="detail-card">
                    ...
                    <p><strong>Image corroboration target:</strong> {html.escape(self._redact_freeform_text(str(record.image_detail_metrics.get('corroboration_target', 'baseline packet')), privacy_level))}</p>
                    <p><strong>Image layout/object cues:</strong> Layout {html.escape(self._join_redacted(record.image_layout_hints, privacy_level, limit=4))} • Objects {html.escape(self._join_redacted(record.image_object_hints, privacy_level, limit=4))} • Quality {html.escape(self._join_redacted(record.image_quality_flags, privacy_level, limit=4))}</p>
                    ...
                    <p><strong>Map answer readiness:</strong> {html.escape(getattr(record, 'map_answer_readiness_label', 'Not answer-ready'))} • {getattr(record, 'map_answer_readiness_score', 0)}% • {html.escape(self._redact_freeform_text(getattr(record, 'map_anchor_status', 'No stable map/location anchor recovered.'), privacy_level, replacement='[REDACTED_LOCATION_CONTEXT]'))}</p>
