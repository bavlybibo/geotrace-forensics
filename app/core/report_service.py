...
class ReportService:
    ...
    def export_pdf(self, records: List[EvidenceRecord], case_id: str, case_name: str) -> Path:
        ...
        story.append(table)
        story.extend([Spacer(1, 10), Paragraph("Methodology & Limits", heading), Paragraph("Workflow: Acquire → Verify → Extract → Correlate → Score → Report. Scores are triage aids and must be confirmed with analyst review.", body), Paragraph("Limits: filesystem times can drift, missing GPS may be normal for exports/screenshots, and parser failures require secondary validation.", body), Paragraph(f"Build identity: {APP_NAME} {APP_VERSION} ({APP_BUILD_CHANNEL})", body), PageBreak(), Paragraph("Deep Technical Appendix", heading)])
        for record in records:
            story.extend([
