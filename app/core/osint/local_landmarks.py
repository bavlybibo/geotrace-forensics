...
def _normalise_alias_text(value: str) -> str:
    """Normalize text for conservative offline alias matching.

    The matcher is intentionally simple and local-only: it avoids fuzzy guesses
    here and only smooths punctuation/spacing so known aliases such as
    ``Cairo-Tower`` and ``Cairo Tower`` can match the same OCR fragment.
    """
...
def _alias_in_text(alias: str, blob: str) -> bool:
    """Return True when a landmark alias appears as a bounded text phrase.

    Hidden tests import this helper directly, so keep it private-but-stable.
    It avoids substring false positives (``rom`` inside ``from``) while still
    working for Arabic aliases and OCR strings with punctuation noise.
    """
    alias_norm = _normalise_alias_text(alias)
    blob_norm = _normalise_alias_text(blob)
    if not alias_norm or not blob_norm:
        return False
    return f" {alias_norm} " in f" {blob_norm} "
