...
def save_osint_cache(cache_dir: Path, record: Any) -> Path | None:
    try:
        ...
        payload = {
            ...
            "location_estimate_label": getattr(record, "location_estimate_label", "Unavailable"),
            "location_estimate_confidence": getattr(record, "location_estimate_confidence", 0),
            "location_estimate_scope": getattr(record, "location_estimate_scope", "no_signal"),
            "location_estimate_source_tier": getattr(record, "location_estimate_source_tier", "no_signal"),
            "location_estimate_summary": getattr(record, "location_estimate_summary", "No stable location estimate was recovered yet."),
            "location_estimate_supporting_signals": getattr(record, "location_estimate_supporting_signals", []),
            "location_estimate_limitations": getattr(record, "location_estimate_limitations", []),
            "location_estimate_next_actions": getattr(record, "location_estimate_next_actions", []),
            "location_estimate_candidates": getattr(record, "location_estimate_candidates", []),
